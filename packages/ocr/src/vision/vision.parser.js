...
exports.parseRemitoText = parseRemitoText;
exports.parseFacturaText = parseFacturaText;
exports.parseRetencionText = parseRetencionText;
// ── Helpers ──────────────────────────────────────────────────────────────────
 ...
 */
function classifyImpuesto(text) {
    if (/Ingresos?\s+Brutos?|I\.?I\.?B\.?B\.?/i.test(text))
        return 'IIBB';
    if (/Ganancias/i.test(text))
        return 'GANANCIAS';
    return '';
}
 ...
 */
function parseRemitoText(rawText) {
 ...
 * Los campos no detectados quedan como string vacío ''.
 */
function parseFacturaText(rawText) {
    const text = normalizeText(rawText);
    return {
 ...
 * Los campos no detectados quedan como string vacío ''.
 */
function parseRetencionText(rawText) {
    const text = normalizeText(rawText);
    // CUIT del Agente de Retención — tres estrategias en orden de confianza
    const cuitEmisor = normalizeCuit(extract(text, RETENCION_PATTERNS.cuitEmisorBeforeRetenido) ||
