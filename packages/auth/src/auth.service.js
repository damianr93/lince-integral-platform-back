...
let AuthService = AuthService_1 = class AuthService {
    constructor(users, jwtService, config) {
        this.users = users;
    ...
    async findOrCreateSuperAdmin(email, password, name) {
        const existing = await this.users.findOne({ where: { email } });
        if (existing)
            return existing;
        const passwordHash = await AuthService_1.hashPassword(password);
...
exports.AuthService = AuthService = AuthService_1 = __decorate([
    ...
    __param(0, (0, typeorm_1.InjectRepository)(database_1.UserEntity)),
