...
const customer_follow_up_service_1 = require("../follow-up/customer-follow-up.service");
...
class CustomValidators {
    static validateMongoId(id, fieldName = 'id') {
        if (!id) {
            throw new common_1.BadRequestException(`El campo '${fieldName}' es requerido`);
        }
        const mongoIdRegex = /^[0-9a-fA-F]{24}$/;
        if (!mongoIdRegex.test(id)) {
            throw new common_1.BadRequestException(`El campo '${fieldName}' debe ser un ID válido de MongoDB`);
        }
        return id;
    }
    static validateEmail(email, fieldName = 'email') {
        ...
        if (!emailRegex.test(email)) {
            throw new common_1.BadRequestException(`El campo '${fieldName}' debe tener un formato de email válido`);
        }
        return email;
    }
    static validatePhone(phone, fieldName = 'telefono') {
        if (!phone)
            return phone;
        let cleanPhone = phone.replace(/[\s\-\(\)]/g, '');
        cleanPhone = CustomValidators.normalizeArgentinePhone(cleanPhone);
        if (!/^\d{8,15}$/.test(cleanPhone)) {
            throw new common_1.BadRequestException(`El campo '${fieldName}' debe contener entre 8 y 15 dígitos`);
        }
        return cleanPhone;
    }
    static normalizeArgentinePhone(phone) {
    ...
    static validateEnum(value, validOptions, fieldName) {
        if (!validOptions.includes(value)) {
            throw new common_1.BadRequestException(`El campo '${fieldName}' debe ser uno de: ${validOptions.join(', ')}`);
        }
        return value;
    }
...
let CustomersService = CustomersService_1 = class CustomersService {
    constructor(clientModel, followUpService, config) {
    ...
    async create(dto) {
        try {
            ...
            if (dto.telefono) {
                ...
                if (existingClient) {
                    isReconsulta = true;
                    this.logger.log(`Reconsulta detectada para el teléfono ${dto.telefono}, cliente original ${existingClient._id}`);
                }
            }
            dto.isReconsulta = isReconsulta;
            const createdCustomer = await this.clientModel.create(dto);
            if (!createdCustomer) {
                throw new common_1.BadRequestException('No se pudo crear el cliente');
            }
            this.logger.log(`Cliente creado exitosamente: ${createdCustomer._id}`);
            await this.scheduleFollowUpSafely(createdCustomer, null);
            ...
            if (error.name === 'ValidationError') {
                throw new common_1.BadRequestException(`Datos de cliente inválidos: ${error.message}`);
            }
            throw new common_1.InternalServerErrorException('Error interno al crear el cliente. Por favor, intente nuevamente.');
        }
    }
    async findAll() {
        try {
            ...
            throw new common_1.InternalServerErrorException('Error al cargar la lista de clientes. Por favor, intente nuevamente.');
        }
    }
    async findOne(id) {
        try {
            ...
            throw new common_1.InternalServerErrorException('Error al buscar el cliente. Por favor, intente nuevamente.');
        }
    }
    async update(id, dto) {
        try {
            ...
            if (dto.telefono) {
                ...
                if (existingClient) {
                    throw new common_1.BadRequestException(`Ya existe otro cliente con el teléfono ${dto.telefono}`);
                }
            }
            ...
            const previousStatus = currentClient.estado;
            ...
            await this.scheduleFollowUpSafely(updated, previousStatus ?? null);
            return updated;
        }
        catch (error) {
            if (error instanceof common_1.NotFoundException || error instanceof common_1.BadRequestException) {
            ...
            throw new common_1.InternalServerErrorException('Error interno al actualizar el cliente. Por favor, intente nuevamente.');
        }
    }
    async remove(id) {
        try {
            ...
            throw new common_1.InternalServerErrorException('Error interno al eliminar el cliente. Por favor, intente nuevamente.');
        }
    }
    /**
     ...
     */
    validateClientData(dto, isUpdate = false) {
        if (dto.nombre !== undefined) {
            dto.nombre = this.cleanCrmData(dto.nombre);
            if (!dto.nombre) {
                throw new common_1.BadRequestException('El nombre es requerido y no puede estar vacío');
            }
        ...
        if (dto.siguiendo !== undefined) {
            ...
            if (dto.siguiendo) {
                const validAdvisors = this.config
                    .get('CRM_ADVISORS', 'EZEQUIEL,DENIS,MARTIN')
                    .split(',')
    ...
    async scheduleFollowUpSafely(customer, previousStatus) {
        if (!customer) {
            return;
        }
        try {
            ...
            const message = error?.message ?? 'Error desconocido al programar seguimiento';
            this.logger.warn(`No se pudo programar el seguimiento automático para el cliente ${customer._id?.toString?.() ?? 'sin-id'}: ${message}`, error?.stack);
        }
    }
    async generateExcel(res) {
        const clients = await this.clientModel.find();
        if (!clients || clients.length === 0) {
            throw new common_1.NotFoundException('No hay clientes para exportar');
        }
        ...
        res.setHeader('Content-Disposition', 'attachment; filename="clientes_completos.xlsx"');
