...
export class AnalyticsService {
  ...
  private async findClientsForReport(filters: LocationFilters): Promise<Customer[]> {
    ...
    return this.clientModel.find(query).lean() as unknown as Promise<Customer[]>;
  }
  ...
  private renderClientTable(doc: PDFKit.PDFDocument, clients: Customer[]) {
    if (clients.length === 0) {
      return;
    }
  ...
  private buildClientRow(client: Customer): string[] {
  ...
  private drawClientDetailRow(doc: PDFKit.PDFDocument, client: Customer, tableWidth: number) {
  ...
  private buildClientDetailLine(client: Customer): string {
    const parts: string[] = [];
  ...
  private buildLocationLabel(
    ubicacion: Customer['ubicacion'] | undefined,
    client: Customer,
  ): string {
    const parts = [
  ...
  private matchesProvince(client: Customer, province: string): boolean {
    const ubicacion = client.ubicacion ?? {};
  ...
  private hasLocation(client: Customer): boolean {
    const ubicacion = client.ubicacion ?? {};
  ...
  private async ensureNormalizedLocations(filters: LocationFilters): Promise<void> {
    ...
    for (let batchIndex = 0; batchIndex < this.normalizationMaxBatches; batchIndex += 1) {
      ...
      for (let i = 0; i < candidates.length; i += this.normalizationConcurrency) {
        ...
        await Promise.all(batch.map((client) => this.normalizeClientLocation(client as unknown as Partial<Customer> & { _id?: any })));
      }
    }
  }

  private async normalizeClientLocation(client: Partial<Customer> & { _id?: any }): Promise<void> {
